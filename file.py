...
class Plate:
    ...
    def save(self):
        Image.fromarray(self.img).convert('RGB').save('plates/'+self.name+'2.jpg')
        ...
        for countour in cnt:
            ...
            if temp_w>10:
                if temp_h>10:
                    ...
                    t_horz = np.sum(rcc == 0, axis=0)
                    t_vert = np.sum(rcc == 0, axis=1)
                    t_error=10000;t_id=0;
                    for l in range(35):
                        horz[l] = [int(x) for x in horz[l]]
                        vert[l] = [int(x) for x in vert[l]]
                        t_x = t_horz - horz[l]
                        t_x = [np.abs(x)*np.abs(x) for x in t_x]
                        e_x = np.sum(t_x)
                        t_y = t_vert - vert[l]
...
class IMGArray:
    ...
    def save(self,name):
        Image.fromarray(self.nparray).convert('RGB').save('img2/'+name+'.jpg')
    ...
    def find_cnt(self):
        _,self.cnt,_ = cv2.findContours(self.temp,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)
...
if __name__ == '__main__':
    numbers = ['15', '13', '15', '16', '17', '21', '27', '29', '30', '36', '37', '42', '50', '51',
               '55', '59', '63', '64', '66']
    #init_plates(numbers)
    load_csv('nn16.csv')
